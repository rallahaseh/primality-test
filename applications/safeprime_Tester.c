...
int safeprime_Tester()
{
	...
	ret_val = converting_input(MRT_prime_params_struct.n, &MRT_prime_params_struct.prec);
	if(ret_val == 2) {
		printf("\nIncorrect input!\n");
		return 0;
	}else if(ret_val == 1){
	...
	if(MRT_prime_params_struct.probably_prime==1) {
        //continue with safe prime testing
		int word_count = MRT_prime_params_struct.prec/32;
		int last_i = word_count-1;
		
		//do minus one operation
		MRT_prime_params_struct.n[last_i] &= 0xFFFFFFFE;
		
		//do /2 operation
		MRT_prime_params_struct.n[last_i] >>= 1; //divide by 2 (shift least significant block)
		...
		for(i=1; i<word_count; i++) {
			...
			if(MRT_prime_params_struct.n[last_i-i] & 0x00000001) 				//if(d%2==1)   (if right most bit of current part is 1)
				MRT_prime_params_struct.n[last_i-(i-1)] |= 0x80000000; 		//sets most left bit of previous block to 1
			MRT_prime_params_struct.n[last_i-i] >>= 1; 						//divides current block by 2
		}
		
		//run primality test again
		ret_val = ioctl(dd, IOCTL_MWMAC_MRT_Prime, &MRT_prime_params_struct);
		if(ret_val != 0)
			printf("Error occured\n");
		...
		//print primality result
		if(MRT_prime_params_struct.probably_prime==1)
			printf("\nResult:\nThe number is safe prime.\n\n");
		else if(MRT_prime_params_struct.probably_prime==0)
			printf("\nResult:\nThe number is not safe prime.\n\n");
    }
	else if(MRT_prime_params_struct.probably_prime==0)
...
int converting_input(unsigned int input_value[], unsigned int *prec){
        ...
        int index = 0; // length of entered string
        int start = 2; // excluding 0 and x
        int total_bits, word_count, last_word, i, j;
        __u32 temp;
	...
	//finding exact start removing initial 0's
